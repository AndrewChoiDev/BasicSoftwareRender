...
impl Bitmap {
    ...
    -> Self {
        ...
        Self {
            pixels : (0..dims.x*dims.y).into_iter().map(|i| [rng.gen_range(0, 255), rng.gen_range(0, 255), rng.gen_range(0, 255), 255]).collect(),
            dims
        }
    }
    ...
    -> Vector2<usize> {
        [(uv.x * self.dims.x as f32 + 0.5f32) as usize,
        (uv.y * self.dims.y as f32 + 0.5f32) as usize].into()
    }
